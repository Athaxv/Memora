...
export function Sidebar() {
  ...
  const collapsedPreference = useSidebarStore((s) => s.collapsedPreference);
  const toggleCollapsedPreference = useSidebarStore((s) => s.toggleCollapsedPreference);
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
    isPending: chatSessionsPending,
    isError: chatSessionsError,
  } = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }: { pageParam: string | null }) => {
      ...
      return (await res.json()) as ChatSessionsPage;
    },
    initialPageParam: null as string | null,
    getNextPageParam: (last) =>
      last.hasMore && last.nextCursor ? last.nextCursor : null,
    staleTime: 30_000,
  });
  ...
  const chatLoading = chatSessionsPending || isFetchingNextPage;
      ...
      {!collapsed && (
        <div className="relative flex h-full min-h-0 flex-1 flex-col border-t border-zinc-200/50 px-2 pt-3 overflow-hidden">
          ...
          <div className="min-h-0 flex-1 space-y-1 overflow-y-auto px-1 pb-2 scroll-smooth overscroll-contain [scrollbar-width:thin] [scrollbar-color:rgba(161,161,170,0.5)_transparent] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:border [&::-webkit-scrollbar-thumb]:border-zinc-50 [&::-webkit-scrollbar-thumb]:bg-zinc-300/80 hover:[&::-webkit-scrollbar-thumb]:bg-zinc-400">
            {chatSessionsError && (
              <p className="px-2 text-[12px] text-red-500">Could not load chats</p>
            )}
            {recentChats.map((chat) => {
              const sessionActive = pathname === "/chat" && activeSessionId === chat.id;
            ...
            {recentChats.length === 0 && !chatLoading && !chatSessionsError && (
