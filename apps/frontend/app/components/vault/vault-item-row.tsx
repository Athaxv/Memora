...
export function VaultItemRow({ id, title, summary, type, sourceUrl, createdAt }: VaultItemRowProps) {
  ...
  return (
    <Link
      href={`/memories/${id}`}
      className="group relative flex items-center gap-4 border-b border-dashed border-zinc-100 px-2 py-4 transition-all hover:bg-zinc-50/80 hover:px-3"
    >
      {/* Icon */}
      <div className="flex h-9 w-9 shrink-0 items-center justify-center border border-zinc-200 bg-white transition-colors group-hover:border-zinc-300">
        {faviconUrl ? (
      ...
      {/* Timestamp */}
      <span className="shrink-0 text-[11px] font-bold uppercase tracking-wider text-zinc-400 group-hover:text-zinc-500 transition-colors">
        {relativeTime(createdAt)}
      </span>

      {/* Hover arrow */}
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity">
